...
(function () {
  ...
  function renderDashboard() {
    ...
    document.getElementById("ingr-dashboard-stat-expired").textContent =
      stats.expired;
    ...
    const expTrend = document.getElementById(
      "ingr-dashboard-stat-expired-trend",
    );
    if (stats.expired > 0) {
    ...
    const urgentContainer = document.getElementById(
      "ingr-dashboard-urgent-list",
    );
    if (urgentContainer) {
      const urgentItems = window.ingredientState.ingredients
        .filter((ingr) => {
          const status = getIngredientStatus(ingr);
          return (
            status === "critical" ||
  ...
  function renderIngredients() {
    ...
    const filtered = window.ingredientState.ingredients.filter((ingr) => {
      const matchesSearch =
        ingr.name.toLowerCase().includes(q) ||
        ingr.code.toLowerCase().includes(q);
    ...
    if (paginatedData.length === 0) {
      ...
      tbody.innerHTML = paginatedData
        .map((ingr) => {
          const category = window.ingredientState.categories.find(
            (c) => c.id === ingr.categoryId,
          );
          ...
          const dateFormatted = new Date(ingr.expiry).toLocaleDateString(
            "vi-VN",
          );
  ...
  function renderCategories() {
    ...
    grid.innerHTML = window.ingredientState.categories
      .map((cat) => {
        // Count ingredients in this category
        const count = window.ingredientState.ingredients.filter(
  ...
  function renderGoodsReceipt() {
    ...
    document.getElementById("ingr-receipt-code").value =
      `GR-${todayStr}-${String(window.ingredientState.history.filter((h) => h.type === "IMPORT").length + 1).padStart(3, "0")}`;
    ...
    if (supplierSelect) {
      supplierSelect.innerHTML =
        ...
        window.ingredientState.suppliers
  ...
  function updateReceiptTotals() {
    ...
    tbody.querySelectorAll("tr").forEach((tr) => {
      ...
      if (select && select.value) {
        ...
        const rawVal =
          parseFloat(totalCell.getAttribute("data-raw-value")) || 0;
        if (rawVal > 0) {
          itemsCount++;
          grandTotal += rawVal;
        }
  ...
  window.calculateAuditDiscrepancy = function (ingrId, sysStock) {
    ...
    diffSpan.textContent =
      (roundedDiff > 0 ? "+" : "") + roundedDiff.toFixed(1);
  ...
  window.quickDisposeExpired = async function (ingrId, currentStock) {
    if (
      !confirm(
        "Bạn có chắc chắn muốn xuất hủy tiêu hủy toàn bộ số lượng của nguyên liệu đã hết hạn này?",
      )
    )
      return;
    ...
    try {
      ...
      window.showToast(
        "Hủy kho thành công",
        "Đã tiêu hủy toàn bộ lô hàng hết hạn.",
        "success",
      );
      renderWarnings();
      renderGlobalSidebarAlerts();
  ...
  function renderSuppliers() {
    ...
    container.innerHTML = window.ingredientState.suppliers
      .map((sup) => {
        return `
  ...
  window.viewSupplierHistory = function (supId, supName) {
    ...
    document.getElementById("ingr-supplier-history-title").textContent =
      `Lịch sử giao hàng: ${supName}`;

    // Get transactions belonging to imports from this supplier
    // Simulating matching transaction logs
    const imports = window.ingredientState.history.filter(
      (h) =>
        (h.type === "IMPORT" && h.details.includes(supName)) ||
        h.details.includes(supId),
    );
  ...
  const closeSupHistBtn = document.getElementById(
    "ingr-supplier-history-close",
  );
  if (closeSupHistBtn) {
  ...
  function renderReports() {
    ...
    document.getElementById("ingr-reports-total-cost").textContent =
      (stats.totalValue * 0.42).toLocaleString("vi-VN") + " đ";
    document.getElementById("ingr-reports-waste-cost").textContent =
      (stats.totalValue * 0.035).toLocaleString("vi-VN") + " đ";
    document.getElementById("ingr-reports-total-orders").textContent =
      window.ingredientState.history.filter((h) => h.type === "IMPORT").length +
      " đơn";
          ...
          datasets: [
            ...
            },
            {
              ...
              data: [
                ...
                5.0,
                7.8,
                (stats.totalValue * 0.035) / 1000000,
              ],
              borderColor: "#ef4444",
    ...
    if (ctxTop) {
      ...
      const sortedIngr = [...window.ingredientState.ingredients]
